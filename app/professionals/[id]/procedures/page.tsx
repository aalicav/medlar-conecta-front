...
export default function ProfessionalProceduresPage({ params }: { params: { id: string } }) {
  ...
  // Carregar dados do profissional e procedimentos
  useEffect(() => {
    const loadData = async () => {
      ...
      try {
        ...
        const professionalData = await fetchResourceById<{data: Professional}>('professionals', params.id as string)
        setProfessional(professionalData?.data)
        
        // Buscar procedimentos já negociados
        const negotiatedResponse = await api.get(`/professionals/${params.id}/procedures`)
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      await updateResource(
        `professionals/${professional.id}/procedures`,
        {
  ...
  if (!professional) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-muted-foreground mt-2">O profissional solicitado não foi encontrado ou foi removido</p>
        <Button onClick={() => router.push("/professionals")} className="mt-4">
  ...
  return (
    <>
      <div className="space-y-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center space-y-4 sm:space-y-0">
          <div>
            ...
            <p className="text-muted-foreground">
              Gerenciar procedimentos do profissional: <span className="font-medium">{professional.name}</span>
              <span className="block text-sm">{professional.professional_type} - {professional.specialty}</span>
            </p>
          </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Procedimentos do Profissional</CardTitle>
            <CardDescription>
              Procedimentos com valores já definidos para este profissional
            </CardDescription>
          </CardHeader>
          <CardContent>
            {procedures.length > 0 ? (
